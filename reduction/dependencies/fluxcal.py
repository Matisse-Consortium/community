...
def get_spectrum_caldb(
    ...
    if min_sep < match_radius * u.deg / 3600.0:
        # match
        print(
            "Calibrator found in the database "
            + caldb_file
            + ": "
        ...
        if "vBoekelDatabase" in caldb_file:
            ...
            if "fitsold" in caldb_file:
                ...
                diam_err_cal = (
                    1000.0 * caldb[-2].data["DIAMETER_ERR"][min_sep_idx]
                )  # mas
            else:
                diam_cal = (
                    1000.0 * caldb["DIAMETERS"].data["DIAMETER"][min_sep_idx]
                )  # mas
                diam_err_cal = (
                    1000.0 * caldb["DIAMETERS"].data["DIAMETER_ERR"][min_sep_idx]
                )  # mas
        elif "calib_spec_db" in caldb_file:
            offset = 2
            if "L" in band:
        ...
        out_lst += [
            caldb[min_sep_idx + offset].header["NAME"],
            diam_cal,
            diam_err_cal,
            wav_cal,
            spectrum_cal,
            ...
            cal_ra_lst[min_sep_idx],
            cal_dec_lst[min_sep_idx],
        ]
        caldb.close()
        return True
    else:
...
def plot_spec(
    ...
    if match:
        ...
        plt.title(
            cal_name_db + ", diam = $%.2f \pm %.2f$ mas" % (diam_cal, diam_err_cal)
        )
        ...
        fig.savefig(
            fig_dir
            + "/"
            + cal_name_db.replace(" ", "_")
            + "_%.1d-%.1d_um.png" % (wl_lim[0], wl_lim[1]),
            dpi=200,
        )
        # plt.show()
...
def fluxcal(
    ...
    airmass_sci = (
        inhdul_sci[0].header["HIERARCH ESO ISS AIRM START"]
        + inhdul_sci[0].header["HIERARCH ESO ISS AIRM END"]
    ) / 2.0
    pwv_sci = (
        inhdul_sci[0].header["HIERARCH ESO ISS AMBI IWV30D START"]
        + inhdul_sci[0].header["HIERARCH ESO ISS AMBI IWV30D END"]
    ) / 2.0
    ...
    airmass_cal = (
        inhdul_cal[0].header["HIERARCH ESO ISS AIRM START"]
        + inhdul_cal[0].header["HIERARCH ESO ISS AIRM END"]
    ) / 2.0
    pwv_cal = (
        inhdul_cal[0].header["HIERARCH ESO ISS AMBI IWV30D START"]
        + inhdul_cal[0].header["HIERARCH ESO ISS AMBI IWV30D END"]
    ) / 2.0
    seeing_cal = (
        inhdul_cal[0].header["HIERARCH ESO ISS AMBI FWHM START"]
        + inhdul_cal[0].header["HIERARCH ESO ISS AMBI FWHM END"]
    ) / 2.0
    tau0_cal = (
        inhdul_cal[0].header["HIERARCH ESO ISS AMBI TAU0 START"]
        + inhdul_cal[0].header["HIERARCH ESO ISS AMBI TAU0 END"]
    ) / 2.0
    ...
    print(
        "SCI: %s, airmass = %.2f CAL: %s, RA = %.6f Dec = %.6f, airmass = %.2f"
    ...
    if do_airmass_correction or do_pwv_correction:
        if not (do_airmass_correction):
            airmass_sci_input = 1.0
            airmass_cal_input = 1.0
        else:
            airmass_sci_input = airmass_sci
            airmass_cal_input = airmass_cal
        if not (do_pwv_correction):
            pwv_sci_input = 0.05
            pwv_cal_input = 0.05
        else:
            pwv_sci_input = pwv_sci
            pwv_cal_input = pwv_cal
        print("Do airmass/PWV correction.")
        ...
        create_skycalc_inputfile(
            fname_in_sci, airmass_sci_input, pwv_sci_input, wmin, wmax, wdelta=dlambda
        )
        print("Start SkyCalc (SCI).")
        os.system(
            skycalc_cli_cmd + " -i " + fname_in_sci + " -o " + fname_skycalc_out_sci
        )
        figpath_out = outputdir + "/skycalc_output_sci_" + tag_sci + ".png"
        plot_skycalc_output(
            fname_skycalc_out_sci, figpath_out, airmass_sci_input, pwv_sci_input
        )
        ...
        create_skycalc_inputfile(
            fname_in_cal, airmass_cal_input, pwv_cal_input, wmin, wmax, wdelta=dlambda
        )
        print("Start SkyCalc (CAL).")
        os.system(
            skycalc_cli_cmd + " -i " + fname_in_cal + " -o " + fname_skycalc_out_cal
        )
        figpath_out = outputdir + "/skycalc_output_cal_" + tag_cal + ".png"
        plot_skycalc_output(
            fname_skycalc_out_cal, figpath_out, airmass_cal_input, pwv_cal_input
        )
    ...
    if match == True:
        ...
        if math.isnan(diam_cal):
            ...
            if mode != "flux":
                ...
                return 4
        print(
            "Diameter = %.1f +/- %.1f mas (%s)" % (diam_cal, diam_err_cal, cal_name_db)
        )
        ...
        wl = np.concatenate(
            (
                np.array([wav_cal_model[0] - (wav_cal_model[1] - wav_cal_model[0])]),
                wav_cal_model,
            )
        )
        wh = np.concatenate(
            (
                wav_cal_model,
                np.array([wav_cal_model[-1] + (wav_cal_model[-1] - wav_cal_model[-2])]),
            )
        )
        wm = (wh + wl) / 2
    ...
    if not math.isnan(spectral_binning) and np.all(np.isfinite(dl_coeffs)):
        ...
        idx = np.logical_and(
            wav_cal_model > 0.99 * np.nanmin(wav_cal),
        ...
        idx = np.logical_and(
            wav_cal_model > np.nanmin(wav_cal), wav_cal_model < np.nanmax(wav_cal)
        )
        if 2.0 * len(wav_cal_model) < np.nansum(idx):
            ...
            wl = np.concatenate(
                (
                    np.array(
                        [wav_cal_model[0] - (wav_cal_model[1] - wav_cal_model[0])]
                    ),
                    wav_cal_model,
                )
            )
            wh = np.concatenate(
                (
                    wav_cal_model,
                    np.array(
                        [wav_cal_model[-1] + (wav_cal_model[-1] - wav_cal_model[-2])]
                    ),
                )
            )
            wm = (wh + wl) / 2
            ...
            for i in range(len(wav_cal_model)):
                ...
                sum = (
                    sum
                    + (wav_bin_upper_cal[wi[0]] - wav_bin_lower[i])
                    * spectrum_cal[wi[0]]
                )
                sum = (
                    sum
                    + (wav_bin_upper[i] - wav_bin_lower_cal[wi[-1]])
                    * spectrum_cal[wi[-1]]
                )
    ...
    if os.path.exists(output_fig_dir):
        ...
        plt.plot(
            wav_sci * 1e6, spectrum_cal_resampled, "-r", label="Resampled total sp."
        )
    ...
    if do_airmass_correction or do_pwv_correction:
        ...
        idx = np.logical_and(
            wav_sci * 1e6 > wl_min + wl_range * 0.1,
            wav_sci * 1e6 < wl_max - wl_range * 0.1,
        )
        range_idx = np.logical_and(idx, ~LM_gap_idx)
        pmin = np.nanmin(
            airmass_correction_factor[range_idx]
        )  # np.nanpercentile(y_new, 10.0)
        pmax = np.nanmax(
            airmass_correction_factor[range_idx]
        )  # np.nanpercentile(y_new, 95.0)
        axc.set_ylim([pmin, pmax])
        axc.set_ylabel("Airmass correction factor")
        axc.set_xlabel("$\lambda$ ($\mu$m)")
        # ax1.set_ylim([0.0,1.0])
        figpath_out = (
            outputdir + "/skycalc_airmass_correction_factor_" + tag_sci_final + ".png"
        )
            ...
            for j in range(len(inhdul_sci["OI_FLUX"].data["FLUXDATA"])):
                ...
                if do_airmass_correction or do_pwv_correction:
                    if calc_spectrum_offset:
                        ...
                        rp_list_cal.append(
                            calc_corr_offset(trans_cal_final, flux_raw_cal, shift_max)
                        )
                        rp_list_sci.append(
                            calc_corr_offset(trans_sci_final, flux_raw_sci, shift_max)
                        )
                ...
                flux_calibrated_sci = (
                    ...
                    * n_exp_cal
                    / n_exp_sci
                    * airmass_correction_factor
                )
                if "L" in band:
                    flux_calibrated_sci = np.flip(flux_calibrated_sci)
                fluxerr_calibrated_sci = (
                    np.abs(flux_raw_sci / flux_raw_cal * n_exp_cal / n_exp_sci)
                    * np.sqrt(
                        (fluxerr_raw_sci * n_exp_cal / n_exp_sci / flux_raw_sci) ** 2
                        + (fluxerr_raw_cal / flux_raw_cal) ** 2
                    )
                    * spectrum_cal_resampled
                )
                if "L" in band:
    ...
    if mode == "corrflux" or mode == "both":
        ...
        for j in range(len(inhdul_sci["OI_VIS"].data["VISAMP"])):
            ...
            for i in range(len(sta_indices_cal)):
                if (
                    (sta_index_sci[0] == sta_indices_cal[i][0])
                    and (sta_index_sci[1] == sta_indices_cal[i][1])
                ) or (
                    (sta_index_sci[0] == sta_indices_cal[i][1])
                    and (sta_index_sci[1] == sta_indices_cal[i][0])
                ):
                    idx_cal = i
                    break
            ...
            vis_cal = (
                2
                * j1(math.pi * diam_cal_rad * spatial_frequency)
            ...
            if do_airmass_correction or do_pwv_correction:
                if calc_spectrum_offset:
                    ...
                    if len(trans_cal_final) == len(corrflux_raw_cal):
                        rp_list_cal.append(
                            calc_corr_offset(
                                trans_cal_final, corrflux_raw_cal, shift_max
                            )
                        )
                        rp_list_sci.append(
                            calc_corr_offset(
                                trans_sci_final, corrflux_raw_sci, shift_max
                            )
                        )

            corrflux_calibrated_sci = (
                corrflux_raw_sci
                / corrflux_raw_cal
                * vis_cal
                * spectrum_cal_resampled
            ...
            corrfluxerr_calibrated_sci = (
                np.abs(corrflux_raw_sci / corrflux_raw_cal)
                * np.sqrt(
                    (corrfluxerr_raw_sci / corrflux_raw_sci) ** 2
                    + (corrfluxerr_raw_cal / corrflux_raw_cal) ** 2
                )
                * vis_cal
                * spectrum_cal_resampled
            )  # uncertainty in calibrator diameter is not taken into account
    ...
    outhdul[0].header["HIERARCH ESO PRO CAL DB NAME"] = (
        cal_name_db,
        "Name of calibrator in cal database.",
    )
    outhdul[0].header["HIERARCH ESO PRO CAL DB DBNAME"] = (
        caldb_file,
        "Name of cal database",
    )
    outhdul[0].header["HIERARCH ESO PRO CAL DB RA"] = (ra_cal_db, "[deg]")
    outhdul[0].header["HIERARCH ESO PRO CAL DB DEC"] = (dec_cal_db, "[deg]")
    outhdul[0].header["HIERARCH ESO PRO CAL DB DIAM"] = (
        diam_cal,
        "Calibrator diameter [mas]",
    )
    outhdul[0].header["HIERARCH ESO PRO CAL DB ERRDIAM"] = (
        diam_err_cal,
        "Error in calibrator diameter [mas]",
    )
    outhdul[0].header["HIERARCH ESO PRO CAL DB SEP"] = (
        min_sep_arcsec,
        "Separation (input coord - calDB coord) [arcsec]",
    )
    ...
    if os.path.exists(output_fig_dir):
        ...
        ax1.set_ylim(
            [0.7 * np.min(spectrum_cal_resampled), 1.1 * np.max(spectrum_cal_resampled)]
        )
...
def get_dlambda(inhdul):
    ...
    if "HAWAII" in header["HIERARCH ESO DET CHIP NAME"]:
        ...
        if "HIGH" in dispname:
            if "+" in dispname:
                dl = np.nan  #!!!!!!!!!!!!!! TO BE IMPLEMENTED !!!!!!!!!!!
            else:
                dl = 0.3  # TO BE CHECKED
    return dl


def get_spectral_binning(inhdul):
    header = inhdul[0].header
    spectral_binning = 7  # new default value, it was np.nan before
    for i in range(1, 17):
...
def get_dl_coeffs(inhdul):
    ...
    if "HAWAII" in header["HIERARCH ESO DET CHIP NAME"]:
        ...
        if "HIGH" in dispname:
            if "+" in dispname:
                dl_coeffs = [np.nan] * 4  #!!!!!!!!!!!!!! TO BE IMPLEMENTED !!!!!!!!!!!
            else:
                dl_coeffs = [
                    -1.08178909e-04,
                    6.44793559e-04,
...
def transform_spectrum_to_real_spectral_resolution(
    ...
    kernel = Gaussian1DKernel(
        stddev=kernel_width_px / (2.0 * np.sqrt(2.0 * np.log(2.0)))
    )
    ...
    f_spec_new = interp1d(
        wl_new, spec_convolved, kind="cubic", fill_value="extrapolate"
    )
...
def calc_corr_offset(spectrum1, spectrum2, shift_max):
    ...
    for k in range(-shift_max, +shift_max):
        if k < 0:
            ...
            rp.append(
                scipy.stats.pearsonr(spectrum1[0 : (Ntr + k)], spectrum2[-k:])[0]
            )  # Pearson's r
        else:
            rp.append(
                scipy.stats.pearsonr(spectrum1[k:], spectrum2[0 : (Ntr - k)])[0]
            )  # Pearson's r
    return rp
